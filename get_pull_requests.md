...
```
  ...
  --only-repos          Only fetch pull requests from repositories
...
environment variable to anything (e.g. `1`) use the cache. The retrieval of
issues starts with `JIRA_TOPLEVEL_FILTER_ID`.
