...
def _process(event):
    ...
    message = f"{user} has {len(data_processor.without_jira)}"
    message += f" of {len(data_processor.with_jira) + len(data_processor.without_jira)}"
    message += f" PRs without a Jira key:\n"
